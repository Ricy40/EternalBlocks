...
public class AlderFoliagePlacer extends ModFoliagePlacer {
    public static final Codec<AlderFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> {
        return m_68573_(instance).and(Codec.intRange(0, 16).fieldOf("height").forGetter((p_242833_0_) -> {
    ...
    protected FoliagePlacerType<?> m_5897_() {
        return ModTreePlacers.ALDER_FOLIAGE;
    }

    protected void m_142539_(LevelSimulatedReader generationReader, BiConsumer<BlockPos, BlockState> posSet, Random random, TreeConfiguration config, int trunkHeight, FoliagePlacer.FoliageAttachment foliage, int foliageHeight, int radius, int offset) {
        BlockPos pos = foliage.m_161451_().m_7949_();
        if (foliage.m_68590_()) {
            placeLeaf(generationReader, posSet, random, config, pos.m_6630_(1));
            ...
            placeLeaf(generationReader, posSet, random, config, pos.m_142082_(-1, 0, 0));
            placeLeaf(generationReader, posSet, random, config, pos.m_142082_(1, 0, 0));
            placeLeaf(generationReader, posSet, random, config, pos.m_142082_(0, 0, -1));
            placeLeaf(generationReader, posSet, random, config, pos.m_142082_(0, 0, 1));
            placeLeaf(generationReader, posSet, random, config, pos.m_6630_(0));
    ...
    public int m_5969_(Random random, int currentWidth, TreeConfiguration config) {
        return 0;
    }

    protected boolean m_7394_(Random random, int x, int y, int z, int radius, boolean bool) {
